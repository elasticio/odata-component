...
const defaultMaxPollingPageSize = 1000;
...
module.exports = class ODataClient {
  ...
  async getObjectsPollingByTimestamp(snapshot) {
    ...
    const maxPollingPageSize = this.cfg.maxPollingPageSize ? parseInt(this.cfg.maxPollingPageSize) : defaultMaxPollingPageSize;
    ...
    if(sinceTime) {
      if(this.cfg.odataVersion < 4) {
        ...
        queryStringParameters.$filter = `${this.cfg.timestampField} gt ${sinceTime}`;
      }
    }

    if(this.cfg.pollingEndTime) {
      if(queryStringParameters.$filter) {
        queryStringParameters.$filter += ' and ';
      }
      if(this.cfg.odataVersion < 4) {
        queryStringParameters.$filter += `${this.cfg.timestampField} lt ${snapshot.castType}'${this.cfg.pollingEndTime}'`;
      } else {
        queryStringParameters.$filter += `${this.cfg.timestampField} lt ${this.cfg.pollingEndTime}`;
      }
    }

    let hasMorePages = true;
    let lastUpdateDateSeen = sinceTime;
    ...
    do {
      ...
      if(this.cfg.singlePagePerInterval === '1' && hasMorePages) {
        return;
      }
    } while (hasMorePages);
  ...
  async upsertObjectById(msg, snapshot) {
    ...
    if (allKeysProvided) {
      let idString;
      if(this.cfg.odataVersion < 4) {
        idString = snapshot.keys.map(key => {
          const keyValue = msg.body[key.name];
          const wrappedKeyValue = key.wrapValueInQuotesInUrls ? `'${keyValue}'` : keyValue;
          return `${key.name}=${wrappedKeyValue}`;
        }).join(',');
      } else {
  ...
  static createObjectFactory(oDataClientClass) {
    return {
      // eslint-disable-next-line no-unused-vars
      process: async function (msg, cfg, snapshot = {}) {
        const client = oDataClientClass.create(this, cfg);
        await client.createObject(msg, snapshot);
      },
      getObjects: this._getListOfObjectsFactory(oDataClientClass),
      getMetaModel: this._getMetadataForEntityTypeFactory(oDataClientClass)
    };
