...
module.exports = class ODataClient {
  ...
  async exportObjects() {
    console.log(`Exporting objects of type ${this.cfg.objectType}`);
    const results = await this.restClient.makeRequest({
        url: this.cfg.objectType,
        method: 'GET'
    });
    console.log(`Fetched ${results.value?results.value.length:'none'} values`);
    results.value.forEach(record => this.emitter.emit('data', messages.newMessageWithBody(record)));
  }
  ...
  static getExportObjectsFactory(oDataClientClass) {
      return {
          process: async function (msg, cfg) {
              const client = oDataClientClass.create(this, cfg);
              await client.exportObjects();
          },
          getObjects: this._getListOfObjectsFactory(oDataClientClass)
      };
  }
