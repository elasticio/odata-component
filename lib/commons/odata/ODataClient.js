...
module.exports = class ODataClient {
      ...
      in: {
        ...
        properties: {
          ['FieldsValue']: {
            type: 'string',
            required: true,
            title: 'Enter object value to delete'
          }
        }
      },
      out: {}
  ...
  async deleteObjectByField(msg) {
    const objectType = this.cfg.objectType;
    const fieldValue = msg.body.FieldsValue;
    console.log(`Will perform delete to ${objectType} with field ${fieldValue}`);
 
    const response = await this.restClient.makeRequest({
    url: `${objectType}(${"'"+fieldValue+"'"})`,
    method: 'GET',
     body: null,
    headers: {
  ...
  console.log("response data url "+ `${objectType}(${"'"+fieldValue+"'"})`);

  this.emitter.emit('data', messages.newMessageWithBody(response));
  ...
  static _getMetaModelForDeleteObjectByFieldFactory(oDataClientClass) {
    return function (cfg) {
      const client = oDataClientClass.create(this, cfg);
      return client.getMetaModelForDelete();
    };
  }
  ...
  static upsertObjectByIdFactoryOld(oDataClientClass) {
    return {
      // eslint-disable-next-line no-unused-vars
      process: async function (msg, cfg, snapshot = {}) {
        const client = oDataClientClass.create(this, cfg);
  ...
  static deleteObjectByIdFactory(oDataClientClass) {
    return {
      process: async function (msg, cfg) {
        const client = oDataClientClass.create(this, cfg);
        await client.deleteObjectByField(msg);
      },
      getObjects: this._getListOfObjectsFactory(oDataClientClass),
      getMetaModel: this._getMetaModelForDeleteObjectByFieldFactory(oDataClientClass)
    };
  }
