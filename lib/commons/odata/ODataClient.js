...
module.exports = class ODataClient {
  ...
  async getObjectsPolling(snapshot) {
    const linkResults = await this.odataClient.getObjectsPollingByDeltaLink(snapshot);
    ...
    this.logger.info(`Next delta link: ${linkResults['@odata.deltaLink']}`);
  }
  ...
  async upsertObjectById(msg, snapshot) {
    ...
    if (allKeysProvided) {
      ...
      this.logger.info(`Will perform update to ${objectType} with id ${idString}`);
