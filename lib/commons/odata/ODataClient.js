...
module.exports = class ODataClient {
  ...
  async getTimestampFields() {
    ...
    const fieldTypes = await getFieldsAndCsdlTypesForEntitySet(csdlString, this.cfg.objectType);

    return Object.entries(fieldTypes)
      // eslint-disable-next-line no-unused-vars
      .filter(([fieldName,csdlType]) => csdlType === 'Edm.DateTime' || csdlType === 'Edm.DateTimeOffset')
      // eslint-disable-next-line no-unused-vars
      .map(([fieldName,csdlType]) => fieldName);
  }
  ...
  async getObjectsPollingByTimestamp(snapshot) {
    ...
    if(this.cfg.odataVersion < 4 && ! snapshot.castType) {
      ...
      const fieldDefs = await this.getTimestampFields();
    ...
    if(snapshot.lastUpdateDateSeen) {
      if(this.cfg.odataVersion < 4) {
        queryStringParameters.$filter = `${this.cfg.timestampField} gt ${snapshot.castType}'${snapshot.lastUpdateDateSeen}'`;
      } else {
    ...
    let pageNumber = 0;
    do {
      console.log(`Fetching page ${pageNumber} of results...`);
      queryStringParameters.$skip = pageNumber * maxPollingPageSize;
      ...
      snapshot.lastUpdateDateSeen = pollingResults.value[pollingResults.value.length - 1][this.cfg.timestampField];
      pageNumber++;
    } while (hasMorePages);
  ...
  static _getTimestampFields(oDataClientClass) {
    return async function (cfg) {
      const client = oDataClientClass.create(this, cfg);
      return client.getTimestampFields();
    }
  }
