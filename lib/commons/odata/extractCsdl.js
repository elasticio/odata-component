...
module.exports.getFieldsAndCsdlTypesForEntitySet= async function(csdlString, entitySetName) {
  const service = await parse(csdlString);
  const matchingEntitySetDefinitions = service.entitySets.filter(es => es.name === entitySetName);
  if(matchingEntitySetDefinitions.length !== 1) {
    throw new Error(`Expected one entity set definition for ${entitySetName}.  Instead received ${matchingEntitySetDefinitions.length}.`);
  }
  const entitySetDefinition = matchingEntitySetDefinitions[0];
  return entitySetDefinition.entityType.properties.reduce((soFar, property) => {
    soFar[property.name] = soFar[property.type];
    return soFar;
  },{});
};
